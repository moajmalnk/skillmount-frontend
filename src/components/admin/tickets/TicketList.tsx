...
export const TicketList = ({ tickets, onSelectTicket }: TicketListProps) => {
  if (tickets.length === 0) {
        ...
        >
          ...
          <div className="col-span-3 flex items-center gap-3">
            <Avatar className="h-8 w-8">
              ...
              <AvatarFallback className="bg-primary/10 text-primary text-xs">{ticket.student.name.substring(0,2)}</AvatarFallback>
            </Avatar>
            <div className="flex flex-col">
              <span className="text-sm font-medium leading-none">{ticket.student.name}</span>
              <span className="text-[10px] text-muted-foreground font-mono mt-1">{ticket.student.id}</span>
            </div>
          </div>
