...
const CategoryBadge = memo(({ category, count, isActive, onClick, delay }: any) => (
  <div
    className={`relative group cursor-pointer animate-elegant-entrance w-full sm:w-72 md:w-80 lg:w-96 flex-shrink-0`}
    style={{ animationDelay: `${delay}ms`, animationFillMode: 'both' }}
    onClick={onClick}
  >
    <WobbleCard containerClassName="h-full" className={`h-full overflow-hidden border rounded-2xl transition-all duration-500 hover:shadow-lg hover:-translate-y-1 bg-card/30 backdrop-blur-sm ${isActive
      ...
      <div className="p-4 sm:p-6 flex flex-col justify-center h-full">
        <div className="flex items-center gap-3">
          <div className={`w-8 h-8 rounded-xl flex-shrink-0 flex items-center justify-center transition-all duration-300 ${isActive ? 'bg-primary/20' : 'bg-muted/50 group-hover:bg-primary/10'
            }`}>
            <HelpCircle className={`w-4 h-4 transition-colors duration-300 ${isActive ? 'text-primary' : 'text-muted-foreground group-hover:text-primary'
              }`} strokeWidth={1.5} />
          </div>
          <div className="flex-1 min-w-0">
            <h3 className={`text-sm sm:text-base font-semibold transition-colors duration-300 truncate ${isActive ? 'text-primary' : 'text-foreground group-hover:text-primary'
              }`}>
              {category}
            </h3>
